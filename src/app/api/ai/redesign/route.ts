...
export async function POST(request: NextRequest) {
  ...
  let body;
  try {
    body = redesignSchema.parse(await request.json());
  } catch { return errorResponse("Invalid request body"); }

  try {
    await ensureAiEntitlement(user!.id, "redesign");
  } catch (err) {
  ...
  try {
    const result = await analyzeDesign(design.image_url, "redesign", body.context);
    await deductCredit(user!.id);
    await logAiUsage(user!.id, "redesign", 2500, "gpt-4o");
    return NextResponse.json({ data: result });
  } catch (err) {
    return errorResponse(err instanceof Error ? err.message : "Redesign analysis failed", 500);
  }
}
